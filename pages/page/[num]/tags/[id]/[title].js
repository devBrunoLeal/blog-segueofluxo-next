...
export async function getServerSideProps(context) {
  ...
  const res = await fetch(
    `https://api.segueofluxo.com/wp-json/wp/v2/posts?tags=${id}&_embed=1&per_page=10&page=${page}`
  );
...
const Tags = ({totalPages,id, totalPost, posts, title, page}) => {
  ...
  const handlePageClick = (data) => {
    ...
    selected++;
    if(selected != page){
      window.location.href = location.origin+"/page/"+selected+"/tags/"+id+"/"+title;
    }
